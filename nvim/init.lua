...
require("lazy").setup({
	...
	-- LSPs are provided by Nix, not Mason. See :help lsp for details.
	{
		...
		config = function()
			...
			-- Server configs. LSP binaries are inherited from the environment/PATH
			-- (or installed via the home-manager module's opt-in options); a server
			-- simply won't attach if its binary isn't found.
			local servers = {
				-- Web
				ts_ls = {},
				html = {},
			...
			-- Merge in any custom servers declared via home-manager (see
			-- neovim_evan_custom.lua, generated by the home-manager module).
			local ok, custom = pcall(require, "neovim_evan_custom")
			if not (ok and type(custom) == "table") then
				custom = {}
			end
			servers = vim.tbl_deep_extend("force", servers, custom.servers or {})
	...
	-- Auto-formats code on save using external formatters (prettier, rustfmt, etc.)
	{
		...
		opts = function()
			...
			-- Merge in custom formatters declared via home-manager.
			local formatters = nil
			local ok, custom = pcall(require, "neovim_evan_custom")
			if ok and type(custom) == "table" then
				formatters_by_ft = vim.tbl_extend("force", formatters_by_ft, custom.formatters_by_ft or {})
				formatters = custom.formatter_defs
			end
